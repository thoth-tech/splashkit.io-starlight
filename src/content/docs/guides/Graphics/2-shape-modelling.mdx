...
```csharp
if (is_moving && selected_shape != null)
{
    var s = selected_shape.Value;
    s.x = MouseX() - s.width / 2;
    s.y = MouseY() - s.height / 2;
    for (int i = 0; i < shapes.Count; i++)
        if (shapes[i].Equals(selected_shape.Value)) { shapes[i] = s; break; }
    selected_shape = s;
}
```
</TabItem>
...
if (is_moving && selected_shape != null)
{
    selected_shape.x = SplashKit.MouseX() - selected_shape.width / 2;
    selected_shape.y = SplashKit.MouseY() - selected_shape.height / 2;
}
```
</TabItem>
</Tabs>
...
```csharp
if (is_resizing && selected_shape != null)
{
    var s = selected_shape.Value;
    s.width = MouseX() - s.x;
    s.height = MouseY() - s.y;
...
if (is_resizing && selected_shape != null)
{
    selected_shape.width = SplashKit.MouseX() - selected_shape.x;
    selected_shape.height = SplashKit.MouseY() - selected_shape.y;
}
```
</TabItem>
</Tabs>
...
OpenWindow("Drawing Test", 800, 600);
List<ShapeModel> shapes = new List<ShapeModel>();
ShapeModel? selected_shape = null;
bool is_moving = false;
bool is_resizing = false;
...
while (!WindowCloseRequested("Drawing Test"))
{
    ...
    if (!is_moving && !is_resizing)
    {
        ...
        {
            selected_shape = GetShapeAt(shapes, MouseX(), MouseY());
        ...
        public static void Main()
        {
            ...
            while (!SplashKit.WindowCloseRequested("Drawing Test"))
            {
                ...
                if (!is_moving && !is_resizing)
                {
                    ...
                    {
                        selected_shape = GetShapeAt(shapes, SplashKit.MouseX(), SplashKit.MouseY());
                        if (selected_shape != null)
                        {
...
```csharp
if (KeyDown(KeyCode.CKey) && !is_connecting)
{
    is_connecting = true;
...
```csharp
if (SplashKit.KeyDown(KeyCode.CKey) && !is_connecting)
{
    is_connecting = true;
    connection_start = null;
}
...
if (is_connecting)
{
    if (MouseDown(MouseButton.LeftButton))
    {
        ...
        {
            connection_start = GetShapeAt(shapes, MouseX(), MouseY());
        }
        else
        {
            ShapeModel connection_end = GetShapeAt(shapes, MouseX(), MouseY());
            if (connection_end != null && connection_end != connection_start)
            {
                Connection connection = new Connection { start = connection_start, end = connection_end };
...
if (is_connecting)
{
    if (SplashKit.MouseDown(MouseButton.LeftButton))
    {
        if (connection_start == null)
        {
            connection_start = GetShapeAt(shapes, SplashKit.MouseX(), SplashKit.MouseY());
        }
        else
        {
            ShapeModel connection_end = GetShapeAt(shapes, SplashKit.MouseX(), SplashKit.MouseY());
            if (connection_end != null && connection_end != connection_start)
            {
                Connection connection = new Connection { start = connection_start, end = connection_end };
                connections.Add(connection);
...
for i in range(len(connections)):
    draw_line(rgb_color(255, 0, 0), connections[i].start.x + connections[i].start.width / 2, connections[i].start.y +
...
ShapeModel selected_shape = null;
ShapeModel connection_start = null;
bool is_moving = false;
bool is_resizing = false;
bool is_connecting = false;
...
while (!WindowCloseRequested("Drawing Test"))
{
    ...
    if (is_moving && selected_shape != null)
    {
        selected_shape.x = MouseX() - selected_shape.width / 2;
        selected_shape.y = MouseY() - selected_shape.height / 2;
    }

    if (is_resizing && selected_shape != null)
    {
        selected_shape.width = MouseX() - selected_shape.x;
        selected_shape.height = MouseY() - selected_shape.y;
    }
    ...
    if (is_connecting && connection_start != null)
    {
        DrawLine(ColorBlack(), connection_start.x + connection_start.width / 2, connection_start.y + connection_start.height / 2, MouseX(), MouseY());
    }
    ...
    public static class Program
    {
        ...
        public static void Main()
        {
            ...
            while (!SplashKit.WindowCloseRequested("Drawing Test"))
            {
                ...
                if (is_connecting && connection_start != null)
                {
                    SplashKit.DrawLine(Color.Black, connection_start.x + connection_start.width / 2, connection_start.y + connection_start.height / 2, SplashKit.MouseX(), SplashKit.MouseY());
                }
...
while not window_close_requested_named("Drawing Test"):
    ...
    if is_connecting and connection_start is not None:
        draw_line(rgb_color(0, 0, 0), connection_start.x + connection_start.width / 2,
