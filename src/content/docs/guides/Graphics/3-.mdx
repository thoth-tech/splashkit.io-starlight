...
#### **Model Design**
We start by designing a `ShapeModel` struct (or class) to represent various basic shapes. This model encapsulates attributes like position, size, color, and shape type. This design adheres to the **Object-Oriented Programming (OOP)** principle of **encapsulation**, where the properties and behaviors of a shape are bundled into a single entity.
...
#### **Shape Creation**
Users can select different shapes from a toolbar and drag them onto the canvas for drawing. We add logic in the code to handle this interaction.
...
for (const auto& shape : shapes)
{
    ...
    {
        fill_triangle(COLOR_RED, shape.x, shape.y, shape.x + shape.width, shape.y, shape.x + shape.width / 2, shape.y - shape.height);
    }
...
In this implementation, we apply the **Factory Method Pattern** to create different shape objects based on user selection. The factory method returns an instance of a specific shape type, which is then stored in a collection. This design not only enhances code flexibility and extensibility but also simplifies shape management and rendering.

With this structure, the program can easily be extended to support additional shape types like circles, lines, etc. You simply need to add logic to handle new shape types in the `ShapeModel` and modify the factory method accordingly.


## Graphical operations 

#### Design Pattern Used
For this part, I employed the **Command Pattern** to handle shape selection and movement. This pattern encapsulates different actions (like selecting and moving shapes) into separate command objects, making the code more modular, easier to maintain, and expandable.
...
##### 2. **Dragging Shapes**
   - Once a shape is selected, the user can drag it around by updating the `X` and `Y` coordinates of the shape to follow the mouse’s movement.
