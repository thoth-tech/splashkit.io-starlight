...
const beyondSkGroupsOutputJson =
  "./scripts/json-files/beyond-splashkit-groups.json"; // Path where the "Beyond SplashKit" groups output JSON files will be saved
...
// ------------------------------------------------------------------------------
function generateIndexPageCategories(
  folderGroupName,
  srcDirectory,
  outputDirectory,
) {
  try {
    ...
    console.log(
      kleur.yellow(`${folderGroupName}: `) +
        kleur.green(
          "Available index page category names have been written to " +
            outputDirectory,
        ),
    ); // Log a message to the console indicating that the JSON file has been created
  } catch (err) {
    console.log(
      kleur.red(`Error writing output files for ${folderGroupName}: `, err),
    );
  }
}
  ...
  folders.forEach((folder) => {
    ...
    try {
      ...
      if (stats.isDirectory()) {
        ...
        if (folder != "README.md") console.log(`${folder} is not a diectory`);
      }
    } catch (err) {
...
// Generate json files with list of names of folders (used for the index pages)
// ------------------------------------------------------------------------------
function generateAvailableFunctionsInGuides(
  folderGroupName,
  srcDirectory,
  outputDirectory,
) {
  try {
    ...
    try {
      ...
      console.log(
        kleur.yellow(`${folderGroupName}: `) +
          kleur.green(`Available functions written to ${outputDirectory}.\n`),
      );
    } catch (err) {
...
generateIndexPageCategories(
  "  Guides Groups",
  guidesDir,
  guidesGrouptsOutputJson,
); // For "Guides"
generateIndexPageCategories(
  "  Beyond SplashKit Groups",
  beyondDir,
  beyondSkGroupsOutputJson,
); // For "Beyond SplashKit"

// ------------------------------------------------------------------------------
// Generate Available Guides Functions
console.log("\nGenerating json file for available functions used in guides...");
generateAvailableFunctionsInGuides(
  "  Guides Functions",
  guidesJsonDir,
  guidesFunctionsJson,
);
