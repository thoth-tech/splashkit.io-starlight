...
walk_script = load_animation_script("walk_cycle", "walk.txt")
...
walk_animation = create_animation(walk_script, "walk")

character_pos = point_at(100, 300)
...
while not quit_requested():
    ...
    draw_text(f"Animation Name: {animation_name(walk_animation)}", color_black(), 10, 30)
    draw_text(f"Current Frame: {animation_current_cell(walk_animation)}", 
             color_black(), 10, 50)
    draw_text(f"Animation Ended: {'Yes' if animation_ended(walk_animation) else 'No'}", 
             color_black(), 10, 70)
