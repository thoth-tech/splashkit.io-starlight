...
void UpdateCameraPosition(double playerX, double playerY)
{
    // Test edge of screen boundaries to adjust the camera
...
while (!QuitRequested())
{
    ...
    if (KeyDown(KeyCode.LeftKey)) playerX -= 3;
    if (KeyDown(KeyCode.RightKey)) playerX += 3;
    if (KeyDown(KeyCode.DownKey)) playerY += 3;
    if (KeyDown(KeyCode.UpKey)) playerY -= 3;
    ...
    DrawText("Camera Position: " + PointToString(Camera.Position), ColorWhite(), 10, 30, OptionToScreen());
