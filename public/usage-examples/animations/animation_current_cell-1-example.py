...
states_script = load_animation_script("character_states", "states.txt")
character_bmp = load_bitmap("character", "character.png")
bitmap_set_cell_details(character_bmp, 64, 64, 8, 2, 16)  # 8x2 grid, 16 frames

# Create animation
character_anim = create_animation(states_script, "idle")

character_pos = point_at(400, 300)
prev_cell = -1
...
while not quit_requested():
    ...
    current_cell = animation_current_cell(character_anim)
    
    # Detect frame changes
    if current_cell != prev_cell:
        write_line(f"Frame changed from {prev_cell} to {current_cell}")
    ...
    draw_text(f"Current Cell: {current_cell}", color_black(), 10, 30)
    ...
    frame_pos = (current_cell * bar_width) // 16
    draw_rectangle(color_red(), bar_x + frame_pos - 2, bar_y, 4, 20)
    
    # Draw frame numbers
    for i in range(0, 17, 4):
        x = bar_x + (i * bar_width) // 16
        draw_text(str(i), color_black(), x - 5, bar_y + 25)
