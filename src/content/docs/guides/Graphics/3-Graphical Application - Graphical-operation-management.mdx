...
In graphic creation and management, the `factory` method design pattern is used. The factory method allows different types of graphic objects to be dynamically generated based on user input. This pattern makes the code more flexible and extensible, while simplifying the management and rendering of graphics. By encapsulating the creation logic of a graph in a factory method, we can more easily support new graph types by simply adding the corresponding creation logic.
...
In graphical operations, the `policy mode` is used to handle shape selection, dragging, and resizing. Policy mode allows us to select different action policies at run time, such as dragging or resizing. This approach makes the code more modular and easier to maintain and extend. By encapsulating different operation policies in separate classes or functions, we can more easily implement and manage different graphical operations.
...
{
    // Loop through all shapes to check if the mouse is within any shape
    for (auto& shape : shapes)
    {
        // Check if the mouse is within the bounds of a rectangle
        if (shape.type == "Rectangle" && x >= shape.x && x <= (shape.x + shape.width) && y >= shape.y && y <= (shape.y + shape.height))
        {
            return &shape;
        }
    }
    return nullptr; // Return null if no shape is selected
}

int main()
{
    open_window("Drawing Test", 800, 600);
    vector<ShapeModel> shapes; // Vector to store all shapes
    ShapeModel* selected_shape = nullptr; // Pointer to hold the selected shape
    bool is_moving = false; // Flag to indicate if a shape is being moved
    bool is_resizing = false; // Flag to indicate if a shape is being resized

    while (!window_close_requested("Drawing Test"))
    {
        process_events(); // Process all events

        // Check if the left mouse button is clicked to start dragging a shape
        if (mouse_down(LEFT_BUTTON) && !is_moving && !is_resizing)
        {
            // Try to select a shape at the mouse position
            selected_shape = get_shape_at(shapes, mouse_x(), mouse_y());
            if (selected_shape != nullptr)
            {
                // If space key is pressed, enter resizing mode
                ...
                {
                    // Otherwise, enter moving mode
                    is_moving = true;
                }
            }
            else
            {
                // If no shape is selected, create a new rectangle at the mouse position
                ShapeModel newShape;
                newShape.x = mouse_x();
                newShape.y = mouse_y();
                newShape.width = 100;
                ...
                shapes.push_back(newShape); // Add the new shape to the vector
            }
        }

        // Move the selected shape to follow the mouse
        ...
        // Resize the selected shape based on the mouse position
        ...
        if (mouse_up(LEFT_BUTTON))
        {
            is_moving = false;
            is_resizing = false;
            selected_shape = nullptr; // Deselect the shape
        }
        ...
        for (const auto& shape : shapes)
        {
            ...
            {
                fill_rectangle(COLOR_RED, shape.x, shape.y, shape.width, shape.height); // Draw the rectangle
            }
        }
