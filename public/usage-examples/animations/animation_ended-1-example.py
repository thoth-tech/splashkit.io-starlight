...
explosion_bmp = load_bitmap("explosion", "explosion.png")
bitmap_set_cell_details(explosion_bmp, 64, 64, 5, 5, 25)  # 5x5 grid, 25 frames

# Create animation that plays once
explosion_anim = create_animation(explosion_script, "explode")

explosion_pos = point_at(400, 300)
...
while not quit_requested():
    ...
    draw_text(f"Explosion Active: {'Yes' if show_explosion else 'No'}", 
             color_white(), 10, 30)
    draw_text(f"Animation Ended: {'Yes' if animation_ended(explosion_anim) else 'No'}", 
             color_white(), 10, 50)
    draw_text(f"Current Frame: {animation_current_cell(explosion_anim)}", 
             color_white(), 10, 70)
