...
In this part, we use [Draw Bitmap](https://splashkit.io/api/graphics/#draw-bitmap) to display the grid and apply zooming. The [Option Scale Bmp](https://splashkit.io/api/graphics/#option-scale-bmp) function scales the bitmap based on the zoom factor, and we display a message in the bottom of the window to guide the user. After drawing the bitmap, we refresh the screen using [Refresh Screen](https://splashkit.io/api/graphics/#refresh-screen).
