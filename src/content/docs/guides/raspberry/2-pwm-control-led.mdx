...
Running this, you may be able to see a noticeable flicker when the frequency is at a low value but it quickly stabilises as it increases. This happens due to the way our eyes perceive light, specifically the [Persistence of Vision](https://en.wikipedia.org/wiki/Persistence_of_vision). If the frequency is faster than the time that images remain in our vision, then we do not notice the times when the LED is unlit.
