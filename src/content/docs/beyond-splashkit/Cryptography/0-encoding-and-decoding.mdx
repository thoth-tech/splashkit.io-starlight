...
description: Explore the essentials of encoding and decoding data using Base64, Caesar Cipher, URL transformations and Hill Cipher for secure and efficient data representation.
category: Guides
author: Brianna Laird and Anna Shustser
...
:::tip[Common Uses of Base64 Encoding]
...
:::tip[Common Uses of URL Encoding and Decoding]
...
The Hill Cipher is a polygraphic substitution cipher based on linear algebra (such as matrix multiplication, determinant calculation, and finding matrix inverses under modular arithmetic).

First, each letter is mapped to a number (e.g. A → 0, B → 1, C → 2, ..., Z → 25). These numbers will be used further to represent the pairs of letters in the form of column matrices.

:::note

To ensure that the input into the function is uniform, all letters are first be converted to uppercase, using `toupper`, which will ensure that the conversions are made within **Modulo 26**, representing the number of letters in the alphabet.

Characters can then be converted to integers and vice versa using the associated ASCII value of the first uppercase character, `A`. This results in the following code:
...
Next, the 2x2 **Key Matrix** is defined. This matrix is usually denoted as K and has the following form:
...
The key matrix is constant and is repetitively used for converting all the letter pairs in the message in the same pattern. As long as the recipients know the Key Matrix, the message can be decoded correctly.

>TODO: Add explanation about input for key matrix (input 4 characters, check mod_inverse using matrix from input characters, etc).
...
### Encoding with the Hill Cipher

>TODO: Improve explanation below and add C++ code for a function named "hill_cipher_encode" - check that key matrix is invertible inside function.

To encoding data with the Hill Cipher, we take the first 2 letters of the message string and convert them to their mapped integer using the `char_to_int` function. Then we complete simple matrix multiplication and use the `int_to_char` function to map the integers back to letters, which are then appended to the string for encoded message.

```cpp
string hill_cipher_encode(string text, string key)
{
    // create key matrix from key string
    // check that key matrix is invertible
    // encode
}
```
...
Here is an example of how you can use the `hill_cipher_encode` function to encode a message:

```cpp
string message = "HELLOWORLD";
string key = "HELP";
string encoded_message = hill_cipher_encode(message, key);
...
### Decoding with the Hill Cipher

TODO: Add explanation and code for a function named "hill_cipher_decode" (can just be C++ code). And any related functions and explanations such as the inverse matrix.
...
```cpp
string hill_cipher_decode(string text, string key)
{
    // create key matrix from key string
    // check that key matrix is invertible
    // convert key to inverse matrix
    // decode
}
```
...
```cpp
string message = "NHRAEQKTLK";
string key = "HELP";
string decoded_message = hill_cipher_decode(message, key);
write_line("Original: " + message);
