...
def main():
    ...
    # Load frog bitmap and configure animation cells
    frog_bmp = load_bitmap("frog_bmp", "Frog.png")
    bitmap_set_cell_details(frog_bmp, 73, 105, 4, 4, 16)

    # Load animation script and create initial animation
    walking_script = load_animation_script("walking_script", "kermit.txt")
    frog_animation = create_animation(walking_script, "WalkFront")
    draw_opt = option_with_animation(frog_animation)

    # Load font for animation label
    default_font = load_font("default", "arial.ttf")

    while not quit_requested():
        process_events()

        clear_screen(color_white())
        draw_bitmap_with_options(frog_bmp, 20, 20, draw_opt)
        draw_text(animation_name(frog_animation), color_black(), default_font, 12, 0, 0)
        refresh_screen()

        update_animation(frog_animation)

        # Handle animation changes via key input
...
def main():
    ...
    while not quit_requested():
        ...
        # Handle animation changes with moonwalk on Left Shift
        if key_typed(KeyCode.up_key):
