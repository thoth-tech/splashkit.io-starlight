...
1. **Install WSL (Ubuntu)**
   ...
   To install both WSL and Ubuntu from the command line, follow these steps. Open Terminal, PowerShell, or Command Prompt in _administrator mode_ by right-clicking and selecting "Run as administrator," then enter the command below:
...
3. **(Optional) Configure Windows Terminal for WSL**
   ...
   To have the Windows **Terminal** app automatically open with WSL, you can set the _Default profile_ to WSL (Ubuntu).
   - Open the Terminal app, click the drop-down arrow at the top of the window, then click "Settings."
   - In _Default profile_, select an **Ubuntu** profile. Choose the profile with the Linux penguin icon if available.
