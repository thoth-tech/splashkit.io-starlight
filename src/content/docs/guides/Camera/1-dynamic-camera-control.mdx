...
Here, we open a 500x550 window using [OpenWindow](https://splashkit.io/api/windows/#open-window) and load a bitmap using [LoadBitmap](https://splashkit.io/api/graphics/#load-bitmap). The [CameraPosition](https://splashkit.io/api/camera/#camera-position) is initially set to (0, 0), and "zoomedIn" is set to false, which will change later when we implement the zoom functionality.
...
Here, we use [Mouse Clicked](https://splashkit.io/api/input/#mouse-clicked) to detect when the user clicks inside the window. If the left mouse button is clicked and the camera is not already zoomed in, we calculate the coordinates of the clicked box by dividing the mouse's position by "BOX_SIZE".
Once we know which box was clicked, we update the camera position. The function set_camera_position [SetCameraPosition](https://splashkit.io/api/camera/#set-camera-position) is used to reposition the camera so that the clicked box is centered in the view. The camera’s position is updated based on the new "camera_pos" values, which take into account both the clicked box and the zoom factor (ZOOM_SCALE). This zooms into the selected area by moving the camera and enlarging the view, creating the zoom-in effect.
...
Here, the [Key Typed](https://splashkit.io/api/input/#key-typed) function is used to detect whether the "ESCAPE_KEY" was pressed. When the "ESCAPE_KEY" is pressed and the view is zoomed in (zoomed_in == true), the camera's position is reset by setting "camera_pos.x" and "camera_pos.y" to 0. This moves the camera back to the original full grid view. The function "set_camera_position(camera_pos)" is then called to update the camera position, and "zoomed_in" is set to false to indicate that the view is no longer zoomed in.
...
In this part, we use [Draw Bitmap](https://splashkit.io/api/graphics/#draw-bitmap) to display the grid and apply zooming if "zoomed_in" is true. The [Option Scale Bmp](https://splashkit.io/api/graphics/#option-scale-bmp) function scales the bitmap based on the zoom factor, and we display a message in the bottom of the window to guide the user. After drawing the bitmap, we refresh the screen using [Refresh Screen](https://splashkit.io/api/graphics/#refresh-screen).
...
The [MouseClicked](https://splashkit.io/api/input/#mouse-clicked) function detects when the left mouse button is clicked. When clicked, "to_world(mouse_position())" converts the mouse's screen position to a position in the game world. This updates the target to the mouse's clicked location.
...
This section detects scrolling through the [Mouse Wheel Scroll](https://splashkit.io/api/input/#mouse-wheel-scroll) function. If the user scrolls in any direction, the camera moves horizontally or vertically based on the scroll input. The target is reset to the center of the screen after scrolling.
