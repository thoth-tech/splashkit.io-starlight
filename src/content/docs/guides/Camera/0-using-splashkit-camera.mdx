...
public class Game
{
    ...
    private const int ScreenBorder = 100;
    private const double PlayerSpeed = 3;

    public Game()
    {
        _window = SplashKit.OpenWindow("Camera Test", ScreenWidth, ScreenHeight);
        _playerX = ScreenWidth / 2;
        _playerY = ScreenHeight / 2;
        _cameraX = 0;
        _cameraY = 0;
    }
    ...
    {
        double leftEdge = _cameraX + ScreenBorder;
        double rightEdge = leftEdge + ScreenWidth - 2 * ScreenBorder;
        double topEdge = _cameraY + ScreenBorder;
        double bottomEdge = topEdge + ScreenHeight - 2 * ScreenBorder;

        if (_playerY < topEdge)
        {
            _cameraY += topEdge - _playerY;
        }
        else if (_playerY > bottomEdge)
        {
            _cameraY += bottomEdge - _playerY;
        }

        if (_playerX < leftEdge)
        {
            _cameraX += leftEdge - _playerX;
        }
        else if (_playerX > rightEdge)
        {
            _cameraX += rightEdge - _playerX;
        }
    }
    ...
    private void Draw()
{
    ...
    // Adjust drawing coordinates based on camera position
    _window.FillRectangle(Color.Gray, 0 - _cameraX, 0 - _cameraY, ScreenWidth, 50);
    _window.DrawText("HUD", Color.White, 10 - _cameraX, 10 - _cameraY);
    _window.DrawText($"Camera Position:{_cameraX:0.00000}:{_cameraY:0.00000}", Color.White, 10 - _cameraX, 30 - _cameraY);

    _window.FillCircle(Color.Yellow, _playerX - _cameraX, _playerY - _cameraY, 20);
    _window.FillRectangle(Color.White, 400 - _cameraX, 200 - _cameraY, 10, 10);
}
