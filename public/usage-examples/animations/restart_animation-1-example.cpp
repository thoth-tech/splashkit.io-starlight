...
int main()
{
    ...
    animation_script dance_script = load_animation_script("dancer", "dancer.txt");
    bitmap dancer_bmp = load_bitmap("dancer", "dancer.png");
    bitmap_set_cell_details(dancer_bmp, 80, 80, 3, 3, 9); // 3x3 grid, 9 frames
    
    // Create animation
    animation dance_anim = create_animation(dance_script, "dance_sequence");
    
    point_2d dancer_pos = {400, 300};
    bool is_playing = true;
    bool show_restart_message = false;
    timer restart_timer = create_timer("restart_timer");
    ...
    while (!quit_requested())
    {
        ...
        if (is_playing && animation_ended(dance_anim))
        {
            restart_animation(dance_anim);
            show_restart_message = true;
            start_timer(restart_timer);
        ...
        if (show_restart_message && timer_ticks(restart_timer) > 1000)
        {
        ...
        string status = is_playing ? "PLAYING" : "STOPPED";
        color status_color = is_playing ? color_green() : color_red();
        draw_text("Status: " + status, status_color, 10, 80);
        draw_text("Frame: " + std::to_string(animation_current_cell(dance_anim)), 
                 color_blue(), 10, 100);
        draw_text("Ended: " + string(animation_ended(dance_anim) ? "Yes" : "No"), 
                 color_purple(), 10, 120);
        ...
        {
            float progress = (float)animation_current_cell(dance_anim) / 8.0f; // 0-8 frames
