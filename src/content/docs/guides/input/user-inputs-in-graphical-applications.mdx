...
description: Introductory guide on reading text and mouse input functionality.
author: Various
...
[Hide Mouse](/api/input/#hide-mouse) hides the mouse from the screen. [Show Mouse](/api/input/#show-mouse-functions) shows the mouse to the screen.
...
There are two versions of the [Show Mouse](/api/input/#show-mouse-with-boolean) function. The first version is shown above. The second version shows or hides the mouse based on its boolean parameter. If true, it shows the mouse, while if false, it hides the mouse from the screen.
