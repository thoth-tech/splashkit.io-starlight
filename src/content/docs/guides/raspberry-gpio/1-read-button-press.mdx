...
{
    ProcessEvents();
    if(RaspiRead(buttonPin) == GpioPinValue.GpioHigh)
    {
...
int main()
{
    ...
    connection pi = remote_raspi_init("button_pi", "192.168.1.100", 8888);
    gpio_pin button_pin = PIN_29;
    gpio_pin led_pin = PIN_11;
    gpio_pin_value led_state = GPIO_LOW;

    remote_raspi_set_mode(pi, button_pin, GPIO_INPUT);
    remote_raspi_set_mode(pi, led_pin, GPIO_OUTPUT);

    remote_raspi_set_pull_up_down(pi, button_pin, PUD_DOWN);
    ...
    while(!any_key_pressed())
    {
        ...
        {
            led_state = remote_raspi_read(pi, led_pin);
            if(led_state == GPIO_LOW)
            {
...
// Connect to remote Raspberry Pi
Connection pi = RemoteRaspiInit("button_pi", "192.168.1.100", 8888);
GpioPin buttonPin = GpioPin.Pin29;
GpioPin ledPin = GpioPin.Pin11;
GpioPinValue ledState = GpioPinValue.GpioLow;

RemoteRaspiSetMode(pi, buttonPin, GpioPinMode.GpioInput);
RemoteRaspiSetMode(pi, ledPin, GpioPinMode.GpioOutput);

RemoteRaspiSetPullUpDown(pi, buttonPin, PullUpDown.PudDown);

OpenWindow("dummy_window", 1, 1);
while(!AnyKeyPressed())
{
    ProcessEvents();
    if(RemoteRaspiRead(pi, buttonPin) == GpioPinValue.GpioHigh)
    {
        ledState = RemoteRaspiRead(pi, ledPin);
        if(ledState == GpioPinValue.GpioLow)
        {
...
namespace RaspberryPiButtonLED
{
    public class Program
    {
        ...
        {
            // Connect to remote Raspberry Pi
            Connection pi = SplashKit.RemoteRaspiInit("button_pi", "192.168.1.100", 8888);
            GpioPin buttonPin = GpioPin.Pin29;
            GpioPin ledPin = GpioPin.Pin11;
            GpioPinValue ledState = GpioPinValue.GpioLow;

            SplashKit.RemoteRaspiSetMode(pi, buttonPin, GpioPinMode.GpioInput);
            SplashKit.RemoteRaspiSetMode(pi, ledPin, GpioPinMode.GpioOutput);

            SplashKit.RemoteRaspiSetPullUpDown(pi, buttonPin, PullUpDown.PudDown);

            SplashKit.OpenWindow("dummy_window", 1, 1);
            while (!SplashKit.AnyKeyPressed())
            {
                SplashKit.ProcessEvents();
                if (SplashKit.RemoteRaspiRead(pi, buttonPin) == GpioPinValue.GpioHigh)
                {
                    ledState = SplashKit.RemoteRaspiRead(pi, ledPin);
                    if (ledState == GpioPinValue.GpioLow)
                    {
