        ...
        functionExampleFiles.forEach((exampleTxtKey) => {
          ...
          if (fileNameToCheck == exampleKey) {
            ...
            if (exampleFiles.includes(fileNameToCheck + ".png")) {
              ...
              testingOutput +=
                kleur.red("\u274C Image/Gif\t\t -> ") + kleur.white(fileNameToCheck + " .png or .gif file\n");
          ...
          languageOrder.forEach((lang) => {
            ...
            let codeFilePath =
              categoryPath + "/" + functionKey + "/" + exampleTxtKey.replaceAll(".txt", languageFileExtensions[lang]);
            ...
            if (languageFiles.length > 0) {
              ...
              const csharpFiles = codeFiles
                .filter((file) => file.endsWith("-top-level.cs") || file.endsWith("-oop.cs"))
                ...
                csharpFiles.forEach((file) => {
                  if (file.includes(exampleKey)) {
                    if (file.includes("-top-level")) {
                      mdxContent += `import ${importTitle}_top_level_${lang} from '${codeFilePath
                        .replaceAll(".cs", "-top-level.cs")
                        .replaceAll("/usage", "/public/usage")}?raw';\n`;
                    }
                    if (file.includes("-oop")) {
                      mdxContent += `import ${importTitle}_oop_${lang} from '${codeFilePath
                        .replaceAll(".cs", "-oop.cs")
                        .replaceAll("/usage", "/public/usage")}?raw';\n`;
                    }
                  }
                });
              } else {
                mdxContent += `import ${importTitle}_${lang} from '${codeFilePath.replaceAll(
                  "/usage",
                  "/public/usage"
                )}?raw';\n`;
              }
