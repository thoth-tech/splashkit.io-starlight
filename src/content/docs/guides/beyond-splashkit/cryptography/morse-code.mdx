...
<Steps>
  1. Convert the text to uppercase to ensure proper dictionary matching. 2. Loop
  through each character in the text and find its Morse code equivalent. 3.
  Combine the Morse code sequences into a single string, separating characters
  with spaces and words with `/`.
</Steps>
...
<Steps>
  1. Split the Morse code input into individual sequences using spaces. 2. Map
  each sequence to its corresponding character from the reversed dictionary. 3.
