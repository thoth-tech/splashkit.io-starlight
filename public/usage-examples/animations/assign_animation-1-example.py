...
def main():
    ...
    player_script = load_animation_script("player_states", "player.txt")
    player_bmp = load_bitmap("player", "player.png")
    bitmap_set_cell_details(player_bmp, 64, 64, 4, 4, 16)  # 4x4 grid, 16 frames
    
    # Create animation starting with idle state
    player_anim = create_animation(player_script, "idle")
    
    player_pos = point_at(400, 300)
    current_state = "idle"
    ...
    while not quit_requested():
        ...
        if key_typed(KeyCode.num_1_key):
            ...
            current_state = "idle"
        elif key_typed(KeyCode.num_2_key):
            assign_animation(player_anim, "walk")
            current_state = "walk"
        elif key_typed(KeyCode.num_3_key):
            assign_animation(player_anim, "run")
            current_state = "run"
        elif key_typed(KeyCode.num_4_key):
        ...
        draw_text(f"Current State: {current_state}", color_red(), 10, 80)
        draw_text(f"Animation: {animation_name(player_anim)}", color_blue(), 10, 100)
        draw_text(f"Frame: {animation_current_cell(player_anim)}", 
                 color_blue(), 10, 120)
        ...
        if current_state == "idle":
            draw_rectangle(color_green(), indicator_x, indicator_y, 50, 20)
        else:
            draw_rectangle(color_gray(), indicator_x, indicator_y, 50, 20)
