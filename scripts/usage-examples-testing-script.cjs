...
function getUniqueFunctionNames(categoryKey, jsonData) {
  ...
  const functionNames = category.functions.map(
    (func) => func.unique_global_name,
  );
...
categories.forEach((categoryKey) => {
  ...
  if (txtFiles.length > 0) {
    ...
    functions.forEach((functionKey) => {
      const functionExampleFiles = txtFiles.filter((file) =>
      ...
      if (functionExampleFiles.length > 0) {
        ...
        let functionTitle = functionKey
          .split("_")
          .map((word) => word.charAt(0).toUpperCase() + word.slice(1))
          .join(" ");

        // Create Function Example heading with link
        const functionURL = getFunctionLink(
          apiJsonData,
          groupName,
          functionKey,
        );
        ...
        const signature = apiJsonData[categoryKey].functions
          .map((func) => func.signature)
          [functionIndex].replaceAll(";", "");
        mdxContent += `:::tip[Note]\n`;
        ...
        functionExampleFiles.forEach((exampleTxtKey) => {
          ...
          if (fileNameToCheck == exampleKey) {
            ...
            let exampleFiles = categoryFiles.filter((file) =>
            ...
            testingOutput +=
              "\n------------------------------------------------\n\n";
            testingOutput +=
              kleur.magenta("Testing") +
              kleur.cyan(" -> " + fileNameToCheck) +
              "\n\n";

            // Text file - check already done above
            testingOutput +=
              kleur.green("\u2705 Text Description\t -> ") +
              kleur.white(fileNameToCheck + ".txt\n");

            // Check for output file (.png or .gif)
            if (exampleFiles.includes(fileNameToCheck + ".png")) {
              testingOutput +=
                kleur.green("\u2705 Image\t\t -> ") +
                kleur.white(fileNameToCheck + ".png\n");
            } else if (exampleFiles.includes(fileNameToCheck + ".gif")) {
              testingOutput +=
                kleur.green("\u2705 Image (Gif)\t\t -> ") +
                kleur.white(fileNameToCheck + ".gif\n");
            } else {
              testingOutput +=
                kleur.red("\u274C Image/Gif\t\t -> ") +
                kleur.white(fileNameToCheck + " .png or .gif file\n");
          ...
          // Description
          let txtFilePath =
            categoryFilePath + "/" + functionKey + "/" + exampleTxtKey;
          ...
          const imageFiles = categoryFiles.filter((file) =>
          ...
          if (imageFiles.length > 0) {
            ...
            if (gifFiles.length > 0) {
              ...
              const webmFiles = categoryFiles.filter((file) =>
                file.endsWith(".webm"),
              );
    ...
    try {
      fs.writeFileSync(
        `./src/content/docs/usage-examples/${name}.mdx`,
        mdxContent,
      );
      console.log(
        kleur.yellow("Usage Examples") +
          kleur.green(
            ` -> ${categoryKey
              .split("_")
              .map((word) => word.charAt(0).toUpperCase() + word.slice(1))
      ...
      success = false;
      console.log(
        kleur.red(`Error writing ${categoryKey} MDX file: `) + `${err.message}`,
      );
      return;
    }
  }
