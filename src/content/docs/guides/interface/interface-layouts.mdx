...
Similarly, we can add a [Text Box](/api/interface/#text-box) by declaring a `string` variable _outside_ the main loop, and then calling `my_string = text_box("id", my_string)`:

![A panel with a button and a text box in it.](/gifs/guides/interface/panel_w_button_and_textbox.gif)

:::tip["id"?]
You have to give a identifier/label for each text box. This is used to identify it when the user interacts with it.
:::
...
int main()
{
    ...
    while (!quit_requested())
    {
        ...
        if (start_panel("My panel", rectangle_from(50,50,200,100)))
        {
            ...
            user_message = text_box("id", user_message);
...
while (!QuitRequested())
{
    ...
    if (StartPanel("My panel", RectangleFrom(50, 50, 200, 100)))
    {
        ...
        userMessage = TextBox("id" userMessage);
        ...
        public static void Main()
        {
            ...
            while (!window.CloseRequested)
            {
                ...
                if (SplashKit.StartPanel("My panel", SplashKit.RectangleFrom(50, 50, 200, 100)))
                {
                    ...
                    userMessage = SplashKit.TextBox("id" userMessage);
...
|string text_box(id, text)      | string text_box("Label:", text, true)   |
