...
namespace DrawTriangleSpinExample
{
    public static class Program
    {
        public static void Main()
        {
            ...
            SplashKit.OpenWindow("Spinning Triangle", WIDTH, HEIGHT);
            ...
            while (!SplashKit.QuitRequested())
            {
                ...
                if (SplashKit.KeyTyped(KeyCode.SpaceKey)) isFilled = !isFilled;
                ...
                double x1 = centerX + radius * Math.Cos(a0), y1 = centerY + radius * Math.Sin(a0);
                double x2 = centerX + radius * Math.Cos(a1), y2 = centerY + radius * Math.Sin(a1);
                double x3 = centerX + radius * Math.Cos(a2), y3 = centerY + radius * Math.Sin(a2);

                // I am drawing the triangle as filled or outlined.
                if (isFilled) SplashKit.FillTriangle(Color.SkyBlue, x1, y1, x2, y2, x3, y3);
                else          SplashKit.DrawTriangle(Color.Navy,    x1, y1, x2, y2, x3, y3);

                // I am drawing the on-screen help.
                SplashKit.DrawText("SPACE toggles fill  ESC quits", Color.Black, 16, 16);

                SplashKit.RefreshScreen();
                SplashKit.Delay(16);   // I am pacing to ~60 FPS.
