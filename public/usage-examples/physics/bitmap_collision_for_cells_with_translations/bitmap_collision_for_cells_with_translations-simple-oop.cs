...
namespace BitmapCollisionsDemo
{
    public class Program
    {
        public static void Main()
        {
            ...
            Matrix2D fileBmpMatrix = SplashKit.TranslationMatrix(fileBmpLoc);
            Matrix2D bugBmpMatrix = SplashKit.TranslationMatrix(bugBmpLoc);
            ...
            if (SplashKit.BitmapCollision(skBmp, 50, skBmpMatrix, fileBmp, 20, fileBmpMatrix))
            {
                SplashKit.WriteLine("SplashKit got a new file!");
            }

            if (SplashKit.BitmapCollision(skBmp, 50, skBmpMatrix, bugBmp, 200, bugBmpMatrix))
            {
