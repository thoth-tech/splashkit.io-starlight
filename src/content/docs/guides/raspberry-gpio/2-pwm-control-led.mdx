...
{
    const int freqs[] = {
        10, 20, 40, 50, 80, 100, 160, 200, 250,
...
namespace RaspberryPiPWMFrequency
{
...
Remote GPIO operations require the Raspberry Pi daemon to be running. Use `remote_raspi_init()` to create a connection to the Raspberry Pi. The connection requires a name, the Raspberry Pi IP address, and the daemon port, which is `8888` by default. The returned connection is then passed as the first argument to the remote PWM functions.
...
RemoteRaspiSetPwmRange(
    pi,
    ledPin,
    pwmRange
);

RemoteRaspiSetPwmDutycycle(
    pi,
    ledPin,
    pwmRange / 2
);

foreach (int frequency in freqs)
{
    Write("At Frequency: ");
    WriteLine(frequency);

    RemoteRaspiSetPwmFrequency(
        pi,
        ledPin,
        frequency
    );
...
RaspiSetPwmFrequency(ledPin, 1000);
RaspiSetPwmRange(ledPin, pwmRange);

for (int i = 0; i <= pwmRange; i += 10)
{
    Write("At Duty Cycle: ");
    WriteLine(i);
    RaspiSetPwmDutycycle(ledPin, i);
    Delay(1000);
}

for (int i = pwmRange; i >= 0; i -= 10)
{
    Write("At Duty Cycle: ");
    WriteLine(i);
    RaspiSetPwmDutycycle(ledPin, i);
...
namespace RaspberryPiPWMDutyCycle
{
    public class Program
    {
        public static void Main()
        {
            ...
            SplashKit.RaspiSetPwmFrequency(ledPin, 1000);
            SplashKit.RaspiSetPwmRange(ledPin, pwmRange);

            for (int i = 0; i <= pwmRange; i += 10)
            {
                SplashKit.Write("At Duty Cycle: ");
                SplashKit.WriteLine(i);
                SplashKit.RaspiSetPwmDutycycle(ledPin, i);
                SplashKit.Delay(1000);
            }
