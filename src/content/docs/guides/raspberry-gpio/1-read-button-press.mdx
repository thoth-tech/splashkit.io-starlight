        ...
        public static void Main()
        {
            ...
            while (!SplashKit.AnyKeyPressed())
            {
                ...
                if (SplashKit.RaspiRead(buttonPin) == GpioPinValue.GpioHigh)
                {
                    ...
                    {
                        SplashKit.RaspiWrite(
                            ledPin,
                            GpioPinValue.GpioHigh
                        );
                    }
                    else
                    {
...
Remote GPIO operations require the Raspberry Pi daemon to be running. Use `remote_raspi_init()` in C++ or `RemoteRaspiInit()` in C# to create a connection to the Raspberry Pi. The connection requires a name, the Raspberry Pi IP address, and the daemon port, which is `8888` by default. The returned connection is then passed as the first argument to the remote GPIO functions.
...
int main()
{
    connection pi = remote_raspi_init(
        "my_pi",
        "PI_IP_ADDRESS",
        8888
    );
    ...
    while(!any_key_pressed())
    {
        ...
        if(remote_raspi_read(pi, button_pin) == GPIO_HIGH)
        {
            led_state = remote_raspi_read(
                pi,
                led_pin
            );
...
while(!AnyKeyPressed())
{
    ...
    if(RemoteRaspiRead(pi, buttonPin) == (GpioPinValue)1)
    {
        ledState = RemoteRaspiRead(
            pi,
            ledPin
        );
...
namespace RaspberryPiButtonLED
{
    public class Program
    {
        public static void Main()
        {
            Connection pi = SplashKit.RemoteRaspiInit(
                "my_pi",
                "PI_IP_ADDRESS",
                8888
            );
            ...
            SplashKit.RemoteRaspiSetMode(
                pi,
                buttonPin,
                GpioPinMode.GpioInput
            );

            SplashKit.RemoteRaspiSetMode(
                pi,
                ledPin,
                GpioPinMode.GpioOutput
            );

            SplashKit.RemoteRaspiSetPullUpDown(
                pi,
                buttonPin,
                PullUpDown.PudDown
            );
            ...
            while (!SplashKit.AnyKeyPressed())
            {
                ...
                if (
                    SplashKit.RemoteRaspiRead(
                        pi,
                        buttonPin
                    ) == GpioPinValue.GpioHigh
                )
                {
                    ledState = SplashKit.RemoteRaspiRead(
                        pi,
                        ledPin
                    );
