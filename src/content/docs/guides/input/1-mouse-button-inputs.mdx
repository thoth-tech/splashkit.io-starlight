...
Here are some functions used to handle mouse button inputs with SplashKit. These functions utilize SplashKit's [Mouse Button](/api/input/#mouse-button) data type to determine if a specified button has been clicked, held down, or released.


### List of Mouse Button Enum Values

The [Mouse Button](/api/input/#mouse-button) enumeration in SplashKit is used to read inputs from the mouse, and mastering its use is essential for developers who want to enable mouse-based controls in their applications.
...
Here is a simple example demonstrating how to use the left and right mouse buttons:
...
Here is an example using vertical scrolling to change the hue of the screen color:
...
Having explored how to handle mouse inputs and movements, let's look at how to
