...
:::note[Remote GPIO support]
This tutorial uses SPI communication with the MCP3008 through `raspi_spi_open()`, `raspi_spi_transfer()`, and `raspi_spi_close()`. Remote equivalents for these SPI operations are not currently available, so this example runs locally on the Raspberry Pi.
:::
